...
abcdSAT -- intermediate simplification:lift, probe, block,distill,eliminate, 
            hyber binary resolve, hidden tautology etc  
***************************************************************************************/
#include <math.h>
#include <stdint.h>
#include <stdio.h>
...
void spf_set_ext_terminate (SPF * spf, void *state, int (*terminate)(void * state))
{
  spf->interrupt.state = state;
  spf->interrupt.function = terminate;
}
...
int s_probe (SPF * spf);
int s_distill (SPF * spf);
int s_decomp (SPF * spf);
int s_transitive_reduction (SPF * spf);
int s_bcp (SPF * spf);
void s_extend (SPF * spf);
int s_terminate (SPF * spf);
...
static int s_simplify(SPF * spf) {
   if (!s_unhide (spf)) return 0;
  if (s_terminate (spf)) return 1;
  if (!s_probe (spf)) return 0;
  if (s_terminate (spf)) return 1;
  if (!s_distill  (spf)) return 0;
  if (s_terminate (spf)) return 1;
  if (!s_lift     (spf)) return 0;
  if (s_terminate (spf)) return 1;
  if (!s_decomp   (spf)) return 0;
  if (s_terminate (spf)) return 1;
  if (!s_transitive_reduction (spf)) return 0;
  if (s_terminate (spf)) return 1;
  if (!s_eliminate(spf)) return 0;
  if (s_terminate (spf)) return 1;
  s_clearMap (spf);
  return 1;
}
...
{
  if (s_bcp (spf)==0) return 0;//unknown
  if (!s_simplify (spf)) return 0;
...
int s_midsimp_solve (SPF * spf) {
  ...
  if (res == 10) s_extend (spf); //10: SATISFIED   20:UNSATISFIED 0:UNKNOWN;
  return res;
}
...
{   
    ...
    vec<Lit> lits;
    int rc,idx, sign, lit, blit, tag, other, other2;
    const int * p, * w, * eow, * c, * top;
    HTS * hts;

    int *Seen=(int *) calloc (spf->nvars, sizeof (int));
    ...
    for (idx = 2; idx < spf->nvars; idx++)
      for (sign = -1; sign <= 1; sign += 2) {
	    ...
	    lits.push( (other > 0) ? mkLit(other-1) : ~mkLit(-other-1) );	
	    Seen[abs(lit)]=Seen[abs(other)]=1;
            if (tag == TRNCS){
                 lits.push( (other2 > 0) ? mkLit(other2-1) : ~mkLit(-other2-1) );
                 Seen[abs(other2)]=1;	
	    }
	    rc=solver->addClause(lits); //close a clause
	    if(rc==false) return 20;//UNSAT;
        }
     }
...
//learnt
  Lir * lir = spf->red;
  top = lir->lits.top;
  for (c = lir->lits.start; c < top; c = p + 1) {
      p = c;
      if (*p >= NOTALIT) continue;
      lits.clear();
      while (*p) {
 ...
 // solver->addClause(lits);
  ...
  for (idx = 2; idx < spf->nvars; idx++){
         if (Seen[idx]) {
               int tmp = s_cval (spf, idx);
               if(tmp==0) continue;
               lits.clear();
               int var=idx-1;
               lits.push((tmp > 0) ? mkLit(var): ~mkLit(var));
	       solver->addClause(lits);
	       continue;
         }
         lits.clear();
	 lits.push(mkLit(idx-1));
         solver->addClause(lits); 
 }
 
 solver->setTermCallback(spf->interrupt.state, spf->interrupt.function);
 vec<Lit> dummy;
 lbool ret = solver->solveLimited2(dummy);
 if(ret==l_True || ret==l_Undef) {
          for (idx = 2; idx < spf->nvars; idx++){
 ...
 delete solver;
 free(Seen);
 if(ret==l_False) return 20;//UNSAT;
  return 0;
}
...
static void fatalExit (const char * msg, ...) {
  printf ("c *** internal fatal error %s in '%s': ", msg, __FILE__);
  /*
  va_list ap;
  va_start (ap, msg);
  vprintf (msg, ap);
...
//static int s_dcmp (SPF * spf, int l, int k) { return 0;}
...
static void s_ddown (SPF * spf, int lit) {
  int parent = lit, child, ppos, cpos;
...
static int s_ecmp (int s, int l, int t, int k) {
  if (s > t) return -1;
  if (s < t) return 1;
  return k - l;
}
...
static void s_eup (SPF * spf, int lit) {
  ...
  while (cpos > 0) {
    ...
    if (s_ecmp (pscr, parent, lscr, lit) >= 0) break;
...
static void s_edown (SPF * spf, int lit) {
  ...
  for (;;) {
    ...
    if (cpos + 1 < size) {
      ...
      if (s_ecmp (cscr, child, rscr, right) < 0)
        cpos++, child = right, cscr = rscr;
    }
    if (s_ecmp (cscr, child, lscr, lit) <= 0) break;
    cposptr = s_epos (spf, child);
    p[ppos] = child;
    *cposptr = ppos;    //child, "up from %d", cpos);
    ppos = cpos;
...
static void s_onflict (SPF * spf, int lit, int red, int lidx) {
  spf->conf.lit = lit;
...
static int s_hbred (int subsumed, int red) {
...
NO_HBR_JUST_F3RCE:
	...
	*l = prev;
	...
	hbred = s_hbred (subsumed, red);// hyper binary resolved %s clause %d %d,
...
static void s_mapstk (int * map, Stk * lits) {
  int * p, * eol;
  eol = lits->top;
...
static void s_mapglue (int * map, Stk * lits) {
  int * p, * eol;
  eol = lits->top;
  for (p = lits->start; p < eol; p++)
...
static void s_maplits (SPF * spf, int * map) {
  s_mapstk (map, &spf->irr);
  s_mapglue (map, &spf->red[0].lits);
}
...
static void s_dcpconnaux (SPF * spf, int red, Stk * s) {
...
static void s_dcpcon (SPF * spf) {
  Lir * lir;
  s_dcpconnaux (spf, 0, &spf->dis.irr.bin);
  s_dcpconnaux (spf, REDCS, &spf->dis.red.bin);
  s_dcpconnaux (spf, 0, &spf->dis.irr.trn);
  s_dcpconnaux (spf, REDCS, &spf->dis.red.trn);
  s_relstk (spf, &spf->dis.irr.bin);
  s_relstk (spf, &spf->dis.irr.trn);
  s_relstk (spf, &spf->dis.red.bin);
  s_relstk (spf, &spf->dis.red.trn);
  s_dcpconnaux (spf, 0, &spf->irr);
  lir = spf->red;
  s_dcpconnaux (spf, REDCS, &lir->lits);
}
...
static int s_cmprepr (int a, int b) 
{ int res;
	...
	if (lit) {
	  ...
	  if (mindfsi == dfsimap[ulit]) {
	    ...
	    for (p = component.top - 1; (other = *p) != lit; p--) {
	      if (s_cmprepr (other, repr) < 0) repr = other;
	    }
...
int s_terminate (SPF * spf) {
  int64_t steps;
  int res;
  if (spf->interrupt.function && spf->interrupt.function (spf->interrupt.state)) return 1;
...
static int s_prbpull (SPF * spf, int lit) {
  AVar * av;
  av = s_avar (spf, lit);
  if (av->mark) return 0;
...
static void s_cleanrepr (Stk * represented, int * repr) {
  int idx;
...
FIRST_OUTER_BRANCH_WIHOUT_INNER_PROBE:
   ...
   //saved %d assignments of 1st inner branch %d in 1st outer branch, s_cntstk (&saved), inner, outer
	...
	if (s_cmprepr (other, repr) < 0) SWAP2 (repr, other);
...
END_OF_SECOND_BRANCH:
	...
	if (abs (lit2) == 1) {
	  ...
	  if (s_cmprepr (repr2, repr1) < 0) SWAP2 (repr1, repr2);
...
CONTINUE:
    s_backtrack (spf, 0);
    s_cleanrepr (&represented[0], reprs[0]);
    s_cleanrepr (&represented[1], reprs[1]);
  }
  if (!spf->mt) {
    for (idx = 2; idx < spf->nvars; idx++)
...
static int s_stamp (SPF * spf, int root, DFPR * dfpr, DFOPF * dfopf,
		     Wtk * work, Stk * units, Stk * sccs, 
...
static int s_unhlca (const DFPR * dfpr, int a, int b) {
  const DFPR * c, * d;
  int u, v, p;
  if (a == b) return a;
...
HBR:
	...
	if (root) {
	  if (root == root1)
	    lca1 = lca1 ? s_unhlca (dfpr, lca1, -lit) : -lit;
	  if (root == root2)
	    lca2 = lca2 ? s_unhlca (dfpr, lca2, -lit) : -lit;
	} else {
	  assert (!lca2);
  ...
  for (rootsonly = 1; rootsonly >= 0; rootsonly--) {
   ...
   //unhiding %s round start %u delta %u mod %u,(rootsonly ? "roots-only": "non-root"), pos, delta, mod);
	...
	if (rootsonly) noimpls++; goto CONTINUE;
      ...
      if (rootsonly) roots++;
      searches++;
      stamp = s_stamp (spf, root, dfpr, dfopf,&work, &units, &sccs, 
                       &visited, stamp, irronly);
      while (!s_mtstk (&units)) {
	lit = s_popstk (&units);
	val = s_val (spf, lit);
	if (val > 0) continue;
